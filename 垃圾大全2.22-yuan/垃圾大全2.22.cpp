...
while(1){
	...
	system("color 8e");
	quicklyout("欢迎来到垃圾大全(2.22)~~~");
	quicklyout("1.玩计算小游戏  2.计算器  3.看时间  4.关机  5.休眠  6.重启  7.FENSONWIM  8.和作者对话  9.逃离系列游戏  10.音乐  11.闹铃  12.W的细胞自动机  0.退出");
	long long a;
	cin>>a;
	if(a==0){
		ShowWindow(hwnd,0); 
	...
	//	WinExec("飞速 计算器2.091.exe",SW_HIDE);
		...
		if(ll==1){
			...
			speak("垃圾大全我从2019开始编，进行过好几次大改，才变成现在这个样子。源码加一起3000吧大概，想要可以QQ给你~\n");
		}
		else if(ll==4){
			speak("再见！");
		}
